...
class ChatMessageStatus(Base):
    ...
    __table_args__ = (
        UniqueConstraint("message_id", "user_id", name="uq_chat_message_statuses_message_user"),
    )
    ...
    email_notified_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        server_default=func.now(),
    ...
    message: Mapped["ChatMessage"] = relationship("ChatMessage", back_populates="statuses")
    user = relationship("User")
